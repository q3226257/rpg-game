...
(function () {
    ...
    Window_Selectable.prototype.processDescription = function () {
        if (this.isCurrentItemDescriptionEnabled()) {
            ...
            // Window_NameInput.prototype.processBack()
        } else {
    ...
    Window_DescriptionEx.prototype.drawTraits = function (x, y, item) {
        ...
        // y = Window_DescriptionEx.prototype.drawRightParameter(x, y, "hahahah:12134");
        //绘制随机特效
        var x_params_names = ["命中率", "闪避率", "暴击率", "躲爆率", "回射率", "反射率", "反击率", "HP回复率", "MP回复率", "T回复率",];
        var s_params_names = ["收益率", "保护效果", "回复效果", "药物效果", "MP损耗率", "TP使用率", "物理伤害率", "魔法伤害率", "最低伤害率", "经验等级"];

        for (let i = 0; i < item.plusSParams.length; i++) {
            if (item.plusSParams[i] !== 0) {
                var pst = s_params_names[i] + ": " + item.plusSParams[i];
                y = this.drawRightParameter(x, y, pst);
            }
        }
        for (let i = 0; i < item.rateSParams.length; i++) {
            if (item.rateSParams[i] !== 1) {
                var rst = s_params_names[i] + "上浮: +" + (item.rateSParams[i] * 100 - 100).toFixed(2) + "%";
                y = this.drawRightParameter(x, y, rst);
            }
        }

        for (let i = 0; i < item.plusXParams.length; i++) {
            if (item.plusXParams[i] !== 0) {
                var pxt = x_params_names[i] + ": " + item.plusXParams[i];
                y = this.drawRightParameter(x, y, pxt);
            }
        }
        for (let i = 0; i < item.rateXParams.length; i++) {
            if (item.rateXParams[i] !== 1) {
                var rxt = x_params_names[i] + "上浮: +" + (item.rateXParams[i] * 100 - 100).toFixed(2) + "%";
                y = this.drawRightParameter(x, y, rxt);
            }
        }

        for (let i = 0; i < item.flatXParams.length; i++) {
            if (item.flatXParams[i] !== 0) {
                var fxt = "额外"+x_params_names[i] + ": " + item.flatXParams[i];
                y = this.drawRightParameter(x, y, fxt);
            }
        }
        for (let i = 0; i < item.flatSParams.length; i++) {
            if (item.flatSParams[i] !== 0) {
                var fst = "额外"+s_params_names[i] + ": " + item.flatSParams[i];
                y = this.drawRightParameter(x, y, fst);
            }
        }
